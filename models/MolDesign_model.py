...
class Moldesign_model(Module):
    ...
    def __init__(self, 
        ...
        self.tokenizer = SmilesTokenizer("./data/vocab.txt")
    ...
    def forward(self, batch, smiles_list, metrics_list, protein_file_list, mode='train', opt_config=None, get_attn_map=False):
        ...
        if opt_config is None:
            ...
            vina = torch.tensor([0 for one in metrics_list], device=device).reshape(-1,1).float()
            ...
            vina = torch.tensor([0 for one in metrics_list], device=device).reshape(-1,1).float() + opt_config["vina"]
        ...
        if mode=='test':
            ...
            if get_attn_map:
                ...
                with open('results/vina{}_qed{}_sa{}_lip{}_logp{}.npy'.format(*list(opt_config.values())), 'wb') as f:
            ...
            for i in range(prediction.shape[0]):
                ...
                for t in range(len(smiles)-1,-1,-1):
                    try: 
                        ...
                        Draw.MolToFile(mol, f"results/test_{i}.png")
                        valid += 1
            ...
            with open("results/smiles.txt", "w") as f:
                f.writelines("\n".join(smiles_list))

            print("the number of invalid molecules: {}".format(invalid), 'valid: {}'.format(valid))
            valid_ratio = valid/(valid+invalid)
            return smiles_list
