...
def get_metric_mol(mol):
    results = {
        'qed': qed(mol), # --> B(128)
        'sa': compute_sa_score(mol), # --> C(128)
        'lipinski': obey_lipinski(mol), # --> D(128)
        'logp': get_logp(mol), # --> E(128)
    }  
    return results



def get_metric(protein_fn, mol, use_vina=True, task_id=None, center=None):
    
    #sim_with_train = SimilarityWithTrain()    
    
    if use_vina:
        results = {
            #'Tanimoto similarity': sim_with_train.get_top_sims(mol),
            'rdmol': mol,
            'qed': qed(mol), # --> B(128)
            'sa': compute_sa_score(mol), # --> C(128)
            'lipinski': obey_lipinski(mol), # --> D(128)
        ...
        if task_id is None:
            task_id = protein_fn.replace("/", "__")
        try:
            vina_task = QVinaDockingTask.from_generated_data(protein_fn , mol, task_id=task_id)
            docking_results = vina_task.run_sync(center = center, patience=None)
            vina = docking_results[0]['affinity']
        except:
            vina = None
            
        results['vina']= vina # --> A(128)  A--> optimize vina
    else:
        results = {
            'mol_smiles': protein_fn,
            'qed': qed(mol), # --> B(128)
            'sa': compute_sa_score(mol), # --> C(128)
            'lipinski': obey_lipinski(mol), # --> D(128)
            'logp': get_logp(mol), # --> E(128)
