...
class QVinaDockingTask(BaseDockingTask):
    ...
    @classmethod
    def from_generated_data(cls, protein_fn, ligand_rdmol, protein_root='./data/crossdocked_pocket10', task_id=get_random_id(), center = None, **kwargs):
    ...
    def __init__(self, pdb_block, ligand_rdmol, conda_env='adt', tmp_dir='./tmp_multi_all', use_uff=True, center=None, task_id = get_random_id()):
    ...
    def run(self, exhaustiveness=4, center=None):
        if center is not None:
            self.center = center
        # /usr/local/ADFRsuite-1.0/bin/prepare_receptor
        commands = """
cd {tmp}
...
obabel {ligand_id}_out.pdbqt -O{ligand_id}_out.sdf -h
    ...
    def run_sync(self, center=None, patience=None):
        
        self.run(center=center)
        # patience = 10
